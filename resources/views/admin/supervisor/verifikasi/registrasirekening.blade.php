    ...
    if (!isset($allNasabah)) {
        $sampleItems = collect([
            (object)[
                ...
                'jabatan' => 'Siswa (RPL)',
                'nis_nip' => '212210045',
                'rekening' => (object)['id' => '320701892003', 'status_akun' => 'pending'],
            ],
            (object)[
                'id' => 2,
                'nama_nasabah' => 'Siti Nurhaliza',
                'jabatan' => 'Siswa (TKJ)',
                'nis_nip' => '212210088',
                'rekening' => (object)['id' => '320701892004', 'status_akun' => 'pending'],
            ],
            (object)[
                'id' => 3,
                'nama_nasabah' => 'Budi Santoso, S.Pd.',
                'jabatan' => 'Guru / GTK',
                'nis_nip' => '198503142010011005',
                'rekening' => (object)['id' => '320701892005', 'status_akun' => 'pending'],
            ],
        ]);
        $page = request('page', 1);
        $allNasabah = new \Illuminate\Pagination\LengthAwarePaginator($sampleItems, $sampleItems->count(), $perPage, $page, [
            'path' => url('/admin/supervisor/verifikasi/registrasi')
        ]);
    }
@endphp
...
<div id="viewTabelData" class="fade-in flex flex-1 flex-col justify-start">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4 px-1">
        <h3 class="text-[20px] sm:text-[22px] font-bold text-gray-800">Verifikasi Registrasi</h3>
        <div class="flex bg-gray-100 p-1 rounded-xl w-full sm:w-[300px]">
            ...
            <a href="{{ url('/admin/supervisor/verifikasi/registrasi') }}" class="flex-1 px-2 sm:px-4 py-2 bg-white rounded-lg shadow-sm text-brand-blue font-bold text-xs sm:text-[13px] text-center transition-all">Registrasi</a>
            <a href="{{ url('/admin/supervisor/verifikasi/transfer') }}" class="flex-1 px-2 sm:px-4 py-2 text-gray-500 font-medium text-xs sm:text-[13px] hover:text-brand-blue transition-colors text-center">Transfer</a>
        </div>
    </div>

    <!-- Table Card / Content -->
    <div class="bg-white rounded-2xl sm:rounded-[20px] shadow-card p-4 md:p-6 w-full flex flex-col">
        <div class="flex flex-col md:flex-row justify-between items-stretch md:items-center gap-3 mb-4 pb-3 border-b border-gray-100">
            <form action="{{ url('/admin/supervisor/verifikasi/registrasi') }}" method="get" class="flex gap-2 items-center w-full md:w-auto">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="text-[14px] text-gray-800 font-medium">
                    <!-- Row 1 -->
                    @forelse ( $allNasabah as $index => $nasabah )
                    <tr class="hover:bg-gray-50/50 transition-colors">
                        ...
                        <td class="py-4 px-2 border-b border-gray-50 text-center status-col">
                        ...
                        <td class="py-4 px-2 border-b border-gray-50 action-col">
                            <div class="flex items-center justify-center gap-2">
                                <a href="{{ url('/admin/supervisor/verifikasi/registrasirekening/detail') }}">
                                ...
                                <button type="button" onclick="openConfirmModal({
                                    ...
                                    onConfirm: () => {
                                        ...
                                        row.querySelector('.status-col').innerHTML = '<span class=\'w-7 h-7 rounded-full bg-green-100 text-green-800 flex items-center justify-center mx-auto\'><i class=\'ph ph-check-circle text-[20px]\'></i></span>';
                                        row.querySelector('.action-col').innerHTML = '<span class=\'text-xs text-green-600 font-bold\'>Aktif</span>';
                                        showToast('Rekening {{ $nasabah->nama_nasabah }} berhasil diaktifkan (Mode Preview)', 'success');
                                    }
                                ...
                                <button type="button" onclick="openConfirmModal({
                                    ...
                                    onConfirm: () => {
                                        ...
                                        showToast('Registrasi {{ $nasabah->nama_nasabah }} berhasil ditolak dan dihapus (Mode Preview)', 'error');
                                    }
                                ...
                                <a href="{{ url('/admin/supervisor/verifikasi/registrasirekening/revisi') }}">
                    ...
                    <tr>
                        <td colspan="6" class="text-center py-8 text-gray-500">
                            Tidak ada data registrasi rekening
                        </td>
                    </tr>
                    @endforelse
                </tbody>
